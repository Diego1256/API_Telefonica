...
//chave antifalsificação do TOKEN
		...
		String token = Jwts.builder().setId("projetoTim").setSubject(usuario.getTelefone()).claim("authorities",
...
// método para ler o conteudo de um TOKEN recebido pela API
	...
	private <T> T getContentFromToken(String token, Function<Claims, T>

	claimsResolver) {
		final Claims claims = Jwts.parser().setSigningKey(JwtSecurity.SECRET.getBytes()).parseClaimsJws(token)
				.getBody();
